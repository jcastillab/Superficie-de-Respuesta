...
datos
url ="https://raw.githubusercontent.com/jcastillab/Superficie-de-Respuesta/main/datos.xlsx"
datos = import(url,which = "datos")
datos
```
...
coef_table <- summary(m)$coefficients %>%
  ...
  mutate(
    `p-valor` = round(`p-valor`, 4),   # 👈 redondear a 4 decimales
...
ft_coef <- flextable(coef_table) %>%
  autofit() %>%
  set_caption("Tabla 1. Coeficientes del modelo cuadrático para el rendimiento de biochar")
...
ft_anova <- flextable(anova_table) %>%
  autofit() %>%
  set_caption("Tabla 2. ANOVA (Type III) del modelo cuadrático")
...
text(st_point[1], st_point[2], labels="Mínimo", pos=4, col="red")
```
...
# 2) Calcula centro y Δ a partir de tus datos reales
...
T_del <- (T_high - T_low)/2        # Δ (half-range) de Temperaturaeratura
t_ctr <- (t_high + t_low)/2        # centro de Tiempoo
t_del <- (t_high - t_low)/2        # Δ de Tiempoo

# 3) Pasa de codificado → real
...
# Parámetros estimados y la bondad del ajuste del modelo cuadrático.
```{r}
summary(m_rsm)
```
...
plot_scatter_trat <- function(database, var,
                              per_T_slopes = TRUE,   # líneas por Temperaturaeratura
  ...
  # Cálculo de rho global y por T (Spearman)
  ...
  p <- ggplot(db, aes(x = .data[[var]], y = Y, color = T, shape = t)) +
    ...
    labs(x = var, y = "Rendimiento [%]",
         ...
         subtitle = glue("Spearman ρ (global) = {round(rho_global, 2)}")) +
    theme_minimal()
  ...
  # Caption con ρ por T si se pide
  if (show_rho_by_T) {
    cap <- paste0(
      "ρ por T: ",
      paste0(levels(db$T),
...
plot_scatter_trat_2 <- function(
  ...
  shade_feasible = TRUE,
  per_T_slopes = TRUE,        # líneas LM por Temperaturaeratura
  show_rho_by_T = TRUE,       # muestra ρ por Temperaturaeratura
  logx = FALSE                # log10 en X (útil para inhibidores)
){
  ...
  if (shade_feasible && (!is.null(x_thresh) || !is.null(y_thresh))) {
    ...
    # Ajustes según combinaciones de lados
...
ggplot(rho_byT, aes(x = factor(T), y = variable, fill = rho)) +
  ...
  labs(x = "Temperaturaeratura (°C)", y = NULL, fill = "ρ Spearman",
       title = "Correlación de Spearman por Temperaturaeratura \n(Rendimiento vs. variables en fase solida)") +
  theme_minimal()
```
